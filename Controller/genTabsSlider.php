...
require_once $_SESSION['shop']['publicPath'] . '/View/html/productsTabsSlider.php';
...
/* ============================================================================================================ 
  MAIN
  ============================================================================================================= */
//$shop, $aGroups, $IN_NumTabsSlider, $IN_NumTabs, $CounGroups, $start
if ($_POST) {
    if (isset($_POST['TrackLoad'])) {
        ...
        genTabsSlider_AJAX($shop, $aGroups, $IN_NumTabsSlider, $IN_NumTabs, $CounGroups, $elem);
    } elseif (isset($_POST['nameGroupUnderScore'])) {
        generateGroupTabSlider_AJAX($_POST['GroupItem'], $_POST['nameGroupUnderScore']);
    }
} else {
    genTabsSlider_PHP($shop, $aGroups, $IN_NumTabsSlider, $IN_NumTabs, $CounGroups, 0);
}
...
function genTabsSlider_PHP($shop, $aGroups, $IN_NumTabsSlider, $IN_NumTabs, $CounGroups, $start) {
    /* ======================================= 
      OBJETO
      ======================================== */
    ...
    if ($IN_NumTabs <= 0 || $IN_NumTabsSlider <= 0) {
        die('Error:Controller/genTabsSlider.php<br/>$IN_NumTabs<=0 || $IN_NumTabsSlider<=0');
    }
...
//    echo '$IN_NumTabs' . $IN_NumTabs . '<br/>';///----DEBUG---
    ...
    $shop->TabsSliderProductsByGroup($start, $groupsSelected, $IN_NumTabs);   //SELECT grupo,codigo_producto,nombre_producto,pvp,pvp_incrementado,imagen,valor_oferta FROM f_random_products WHERE grupo = :group;
    ...
    while ($VarCantTabsSlider > 0 && $IN_NumTabs > 0 && ($Cnt * $IN_NumTabs) <= ($CounGroups - 1)) {//Si se pone variable resto pone todos los grupos    
        ...
        G_TBS(); //CIERRA <section> HTML productsTabsSlider.php
    }
    
    //HTML <div id = "Main"> CERRADO View/html/footer.php
}

;

function genTabsSlider_AJAX($shop, $aGroups, $IN_NumTabsSlider, $IN_NumTabs, $CounGroups, $start) {
    /* ======================================= 
      OBJETO
      ======================================== */
    ...
    $shop->TabsSliderProductsByGroup($start, $start + 1, $IN_NumTabs);   //SELECT grupo,codigo_producto,nombre_producto,pvp,pvp_incrementado,imagen,valor_oferta FROM f_random_products WHERE grupo = :group;
    $aProductsByGroups = $shop->getArrAux();
    ...
    A_TBS($aGroups[$start]['grupo']); //ABRE <section> HTML productsTabsSlider.php
...
function generateGroupTabSlider_AJAX($aGroupProducts, $nameGroupUnderScore) {
    // D_TBS();
//-----LEVEL 2--INSIDE----//
    E_TBS($nameGroupUnderScore); //ABRE div.tab-content HTML productsTabsSlider.php   
...
//echo $aGroups['grupo'];
    ...
    for ($i = $Inicio; $i < $Actual; $i++) {        ///0-->4///4-->8///8-->12///12-->16///16-->18
        ...
        B_TBS($FirstLoop, $NoSpaces, $aGroups[$i]['grupo']); //ABRE ul.nav-tabs HTML productsTabsSlider.php
    }
    C_TBS();
